...
const ResultsPanel: React.FC<ResultsPanelProps> = ({ results }) => {
  ...
  return (
    <div className="border border-gray-300 rounded text-xs flex flex-col h-full overflow-hidden">
      {/* Tabs */}
      <div className="flex bg-gray-200 text-sm uppercase font-semibold shrink-0">
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="mb-2">
              <button
                ...
                className="bg-orange-600 text-white px-4 py-1 rounded text-xs hover:bg-orange-700"
                  ...
                  <tbody>
                    {results.map((row) => (
                      <tr key={row.id} className="border-t">
                        ...
                        <td className="text-orange-800 underline">
                          {row.model}
                        </td>
                        <td>{row.cost}</td>
                        <td>{row.load}</td>
                        ...
                        <td
                          dangerouslySetInnerHTML={{
                            __html: row.faceFasteners,
                          }}
                        />
                        <td
                          dangerouslySetInnerHTML={{
                            __html: row.joistFasteners,
                          }}
                        />
                      </tr>
                    ))}
                  ...
                  <tbody>
                    {jobList.map((job) => (
                      <tr key={job.id} className="border-t">
                        ...
                        <td
                          dangerouslySetInnerHTML={{
                            __html: job.faceFasteners,
                          }}
                        />
                        <td
                          dangerouslySetInnerHTML={{
                            __html: job.joistFasteners,
                          }}
                        />
                        <td>
                          <button
